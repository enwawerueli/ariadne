...
def make_executable_schema(
    ...
    *bindables: Union[SchemaBindable, List[SchemaBindable]],
    ...
    kwargs_to_snake_case: bool = False
) -> GraphQLSchema:
    ...
    if kwargs_to_snake_case:
        set_outname_to_snake_case(schema)
...
def set_outname_to_snake_case(schema) -> None:
    for graphql_type in schema.type_map.values():
        if not isinstance(graphql_type, (GraphQLObjectType, GraphQLInputObjectType)):
            continue
